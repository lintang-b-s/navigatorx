...
func (uc *NavigationService) ShortestPathETA(ctx context.Context, srcLat, srcLon float64,
	...
	pN, p, eta, found, dist := alg.AStarCH(fromSurakartaNode, toSurakartaNode)
	...
	return p, dist, n, found, route, eta, nil
}
...
func (uc *NavigationService) ShortestPathAlternativeStreetETA(ctx context.Context, srcLat, srcLon float64,
	...
	go func() {
		...
		defer wg.Done()
		pN, _, eta, found, dist := alg.AStarCH(fromSurakartaNode, alternativeStreetSurakartaNode)
	...
	go func() {
		...
		defer wg.Done()
		pN, _, eta, found, dist := alg.AStarCH(alternativeStreetSurakartaNode, toSurakartaNode)
		pathChan <- ShortestPathResult{
			Paths: pN,
			ETA:   eta,
			Found: found,
...
func (uc *NavigationService) ShortestPathETACH(ctx context.Context, srcLat, srcLon float64,
	...
	var err error
	// fromSurakartaNode, err := alg.SnapLocationToRoadNetworkNodeRtreeCH(from.Lat, from.Lon, "f")
	// if err != nil {
	// 	return "",[]alg.Navigation{}, []alg.Coordinate{}, 0.0, nil
	// }
	// toSurakartaNode, err := alg.SnapLocationToRoadNetworkNodeRtreeCH(to.Lat, to.Lon, "b")
	// if err != nil {
	// 	return "",[]alg.Navigation{}, []alg.Coordinate{}, 0.0, nil
	// }
	fromSurakartaNode, err := alg.SnapLocationToRoadNetworkNodeRtree(from.Lat, from.Lon)
	...
	// pN, p, eta, found, dist := alg.AStarCH(fromSurakartaNode, toSurakartaNode)
	p, eta, dist  := alg.ShortestPathBiDijkstra(fromSurakartaNode, toSurakartaNode)
